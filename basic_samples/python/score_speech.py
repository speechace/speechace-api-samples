...
text is required. The API returns pronunciation, fluency, grammar,
vocabulary, and coherence metrics across multiple scoring frameworks.

Usage:
    python score_speech.py --audio ../../audio_samples/apple.wav
...
def build_url(endpoint: str, key: str, dialect: str, user_id: str) -> str:
    ...
    return f"{base}/api/scoring/speech/v9/json?{params}"


def score_speech(audio_path: str, dialect: str = "en-us") -> dict:
    """Send an audio file to Speechace for speech scoring."""
...
def print_summary(result: dict) -> None:
    """Print a human-readable summary of the speech scoring response."""
    ...
    transcript = score_info.get("transcript")
    if transcript:
...
def main():
    parser = argparse.ArgumentParser(description="Speechace speech scoring sample")
    ...
    try:
        result = score_speech(args.audio, args.dialect)
